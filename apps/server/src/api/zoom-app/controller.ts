...
// In-Client OAuth 2/2
export const inClientOnAuthorized: express.RequestHandler = async (req, res, next) => {
  console.log(
...
export const install: express.RequestHandler = (req, res) => {
  console.log(
    'INSTALL HANDLER ==========================================================',
    '\n'
  );
...
// This route is called after the user has authorized the Zoom App on the market.
export const auth: express.RequestHandler = async (req, res, next) => {
  ...
  const zoomAuthorizationCode = req.query.code as string;
...
export const home: express.RequestHandler = (req, res, next) => {
  ...
  try {
    ...
    console.log("0. Decrypt the Zoom App context header");

    console.log("0a. Get 'x-zoom-app-context'");
    if (!req.headers['x-zoom-app-context']) {
      throw new Error('x-zoom-app-context header is required');
    }

    console.log("0b. Try to decrypt zoom app context ", req.headers['x-zoom-app-context']);
    const decryptedAppContext = zoomHelpers.decryptZoomAppContext(
      req.headers['x-zoom-app-context'] as string,
